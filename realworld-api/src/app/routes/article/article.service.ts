...
const pubsub = new PubSub({
  projectId: process.env.GOOGLE_CLOUD_PROJECT || 'pcd-review-system',
});

const reviewSubmittedTopic =
  process.env.PUBSUB_REVIEW_SUBMITTED_TOPIC || 'review-submitted';
...
export const addComment = async (body: string, slug: string, id: number) => {
 ...
 await firestore.collection('reviews').doc(String(comment.id)).set({
  reviewId: comment.id,
  body: comment.body,
  status: 'pending',
  ...
  await pubsub.topic(reviewSubmittedTopic).publishMessage({
    json: {
      reviewId: comment.id,
